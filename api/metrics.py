...
# Counters
EVALS_SUBMITTED = Counter(
    "evalforge_evals_submitted_total", "Total evaluations submitted", ["environment"]
)
EVALS_COMPLETED = Counter(
    "evalforge_evals_completed_total", "Total evaluations completed", ["environment", "status"]
)
HTTP_REQUESTS = Counter(
    "evalforge_http_requests_total", "Total HTTP requests", ["method", "path", "status"]
)

# Histograms
EVAL_DURATION = Histogram(
    "evalforge_eval_duration_seconds",
    "Evaluation run duration",
    ["environment"],
    buckets=[1, 5, 10, 30, 60, 120, 300, 600],
)
REQUEST_DURATION = Histogram(
    "evalforge_request_duration_seconds",
